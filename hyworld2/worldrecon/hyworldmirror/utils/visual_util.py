...
def download_file_from_url(url, filename, max_retries=3):
    """Downloads a file from a URL with retry and resume support.

    Handles:
    - HTTP redirects (302) from HuggingFace/hf-mirror CDN
    - Incomplete downloads (truncated files) via retry + delete on failure
    ...
    for attempt in range(max_retries):
        try:
            # First pass: resolve redirect to get final URL
            session = requests.Session()
            session.max_redirects = 10
            resp = session.get(url, allow_redirects=True, stream=True, timeout=60)
            ...
            if _os.path.exists(filename):
                existing_size = _os.path.getsize(filename)
                if existing_size >= total_size and total_size > 0:
                    print(f"{filename} already downloaded ({existing_size} bytes).")
                    return
                # Partial file found — delete and restart
                if existing_size > 0 and (total_size == 0 or existing_size < total_size):
                    print(f"Removing corrupted partial download ({existing_size} bytes), retrying...")
                    _os.remove(filename)
                    existing_size = 0
            ...
            # Second pass: download with optional resume
            ...
            final_size = _os.path.getsize(filename)
            if total_size > 0 and final_size != total_size:
                raise IOError(f"Incomplete download: expected {total_size}, got {final_size}")

            print(f"Downloaded {filename} successfully ({final_size} bytes).")
            return
            ...
            if attempt == max_retries - 1:
                print(f"Error downloading {filename} after {max_retries} attempts.")
